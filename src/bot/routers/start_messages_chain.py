...
from aiogram.types import CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, FSInputFile, WebAppInfo
...
chain_messages: list[SendMessage | list[SendMessage]] = [
    SendMessage(text='MESSAGE_1',
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    ...
                    [InlineKeyboardButton(text='NEXT_BUTTON_2', web_app=WebAppInfo(url=settings.WEBAPP_URL))]]
                )),
...
async def chain_messages_handle(
    ...
    if isinstance(message, (list, tuple)):
        for m in message:
            await m.send(chat_id=call.message.chat.id, state=state, texts=texts)
    else:
